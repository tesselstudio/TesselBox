...
func testBlockSystem() {
	...
	allBlocks := registry.GetAllBlocks()
...
func testCraftingSystem() {
	...
	// Test creating items - GetItem returns (*Item, bool)
	woodItem, ok := itemRegistry.GetItem("wood")
	if !ok {
		log.Printf("  ✗ Could not get wood item")
		return
	}

	stoneItem, ok := itemRegistry.GetItem("stone")
	if !ok {
		log.Printf("  ✗ Could not get stone item")
		return
	}
