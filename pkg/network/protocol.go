...
func (p *Protocol) DecodePlayerMove(data []byte) (*PlayerMoveMessage, error) {
	...
	posX := float32(binary.BigEndian.Uint32(data[4:8])) / 1000
	posY := float32(binary.BigEndian.Uint32(data[8:12])) / 1000
	posZ := float32(binary.BigEndian.Uint32(data[12:16])) / 1000

	rotX := float32(binary.BigEndian.Uint32(data[16:20])) / 1000
	rotY := float32(binary.BigEndian.Uint32(data[20:24])) / 1000
	rotZ := float32(binary.BigEndian.Uint32(data[24:28])) / 1000

	velX := float32(binary.BigEndian.Uint32(data[28:32])) / 1000
	velY := float32(binary.BigEndian.Uint32(data[32:36])) / 1000
	velZ := float32(binary.BigEndian.Uint32(data[36:40])) / 1000

	return &PlayerMoveMessage{
		PlayerID: playerID,
		Position: types.NewVec3(posX, posY, posZ),
		Rotation: types.NewVec3(rotX, rotY, rotZ),
		Velocity: types.NewVec3(velX, velY, velZ),
	}, nil
}
...
func (p *Protocol) DecodeBlockPlace(data []byte) (*BlockPlaceMessage, error) {
	...
	posX := float32(binary.BigEndian.Uint32(data[5:9])) / 1000
	posY := float32(binary.BigEndian.Uint32(data[9:13])) / 1000
	posZ := float32(binary.BigEndian.Uint32(data[13:17])) / 1000
